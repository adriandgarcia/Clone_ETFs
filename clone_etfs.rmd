...
I come up with 149 funds that match. I can then cross reference that list against the list I created above to add the ones I'm missing. I go from 408 matches to 502 matches.
...
aa <- Combined_matches %>% 
  ...
  mutate(ETF = sum(ETF, na.rm = TRUE),
         `MUTUAL FUND` = sum(`MUTUAL FUND`, na.rm = TRUE)) %>% 
  select(Strategy.Name, ETF, `MUTUAL FUND`, everything())
...
bb <- Combined_matches %>% 
  ...
  mutate(ETF = sum(ETF, na.rm = TRUE),
         `MUTUAL FUND` = sum(`MUTUAL FUND`, na.rm = TRUE)) %>% 
  select(Name, Name.Mod, Strategy.Name, ETF, `MUTUAL FUND`, everything())
